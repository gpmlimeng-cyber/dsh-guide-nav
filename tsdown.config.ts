...
/**
 * Host 半边：Node 库，输出 lib/，供 cordis.patch.yml 的插件行按包名加载。
 *
 * `@deepseek-ai/*` 一律保持 external：它们是 harness 自己提供的运行时依赖
 * （profile 的 node_modules 里就有），内联会把 cordis 与服务基类复制一份，
 * 导致 `instanceof` 判断与装饰器元数据在两个副本间失配——Remote 端点会因此
 * 注册不上。只有第三方纯函数库才考虑内联。
 */
...
const client = {
  ...
  outputOptions: {
    ...
    banner: 'window.__ModuleLoader__.load({ id: "dsh-guide-nav", factory: (require) => {',
